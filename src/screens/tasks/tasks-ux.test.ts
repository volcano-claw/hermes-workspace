...
describe('tasks UX copy', () => {
  ...
  it('groups tasks by label and treats deleted projected cards as archive', () => {
    ...
    const completedCurrent = task({ id: 'completed-current', group_label: 'A', column: 'done', visibility_state: 'completed_current', position: 3 })
    const groups = groupTasksByLabel([active, archived, completedCurrent])
    expect(groups).toHaveLength(1)
    expect(groups[0].tasks.map(t => t.id)).toEqual(['archived', 'active', 'completed-current'])
    expect(isArchivedTask(active)).toBe(false)
    expect(isArchivedTask(completedCurrent)).toBe(false)
